...
const Blogs = () => {
  ...
  const [date, setDate] = useState(state ? new Date(state.row.date) : null);
  ...
  const handleSubmit = () => {
    ...
    if (state) formData.append("id", state.row.id);
    for (var pair of formData.entries()) {
      console.log(pair[0] + " : " + pair[1]);
    }
    dispatch(
      state
        ? contentActions.editContent(formData)
        : contentActions.postContent(formData)
    );
      ...
      {(formik) => (
        ...
        >
          ...
          <TextField
            ...
            helperText={formik.touched.category ? formik.errors.category : null}
          />
          <TextField
            id="author"
            className={classes.formField}
